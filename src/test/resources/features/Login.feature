Feature: Login
  As user,i want to able to be login into vytrack
  under different roles with username and password
#Any step that was implemented before can be reused
 #if test step has yellow background that means it does not implementation yet
  #click command+option+L to organize code ,save like in java
  #i had somewhwre from 2 to 25 scenarios in evry feature file
  ...
  @negative_test
  Scenario: Verify that warning message that displayed when username is not correct
    Given user is on the login page
